...
export const createUser = (values: Record<string, any>) =>
	new UserModel(values).save().then((user) => user.toObject()); //Creates a new user using the provided values and saves it to the database
export const deleteUserById = (id: string) =>
	UserModel.findOneAndDelete({ _id: id }); //Deletes a user by their ID.
export const updateUserById = (id: string, values: Record<string, any>) =>
	UserModel.findByIdAndUpdate({ id, values }); //Updates a user by their ID with the provided values
